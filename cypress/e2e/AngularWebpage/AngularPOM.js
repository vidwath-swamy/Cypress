class AngularPOM {
    ...
    Shop(){
        cy.contains('Shop',{timeout:10000}).click()
    }
    ...
    Deliverylocation() {
        ...
        // Wait for the suggestions to appear and click the first suggestion
        cy.get('.suggestions', { timeout: 20000 }).should('be.visible').first().click();
    }
    ...
    TermsandConditionsPrompt(){
        cy.get('.nsm-dialog-animation-fade').then(($element) => {
            // Debugger is hit after the cy.visit
            // and cy.get commands have completed
            debugger})//.should('be.visible').and('contain.text','Terms And Conditions')
    }
    ...
    SelectCountryPage(){
        cy.xpath("//label[@for='country']").should('contain.text','Please choose your delivery location.')
    }
}

export default AngularPOM;